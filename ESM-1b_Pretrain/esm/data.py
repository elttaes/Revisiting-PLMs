...
class MSABatchConverter(BatchConverter):
    def __call__(self, inputs: Union[Sequence[RawMSA], RawMSA]):
        if isinstance(inputs[0][0], str):
            ...
            # 判断是不是一个单独的msa，没有batch_size那个维度
            raw_batch: Sequence[RawMSA] = [inputs]  # type: ignore
        else:
            raw_batch = inputs  # type: ignore

        batch_size = len(raw_batch)
        max_alignments = max(len(msa) for msa in raw_batch)  # 每个msa有多少条氨基酸序列
        max_seqlen = max(len(msa[0][1]) for msa in raw_batch) # 每个氨基酸序列最大的氨基酸数目
        ...
        tokens = torch.empty(
            (
                ...
                max_alignments,
                max_seqlen
                + int(self.alphabet.prepend_bos) # True
                + int(self.alphabet.append_eos), # False
            ),
            dtype=torch.int64,
        )
