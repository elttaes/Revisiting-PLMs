...
class ProteinBertModel(nn.Module):
	...
	def forward(self, tokens, repr_layers=[], input_embeddings=None, lengths=None, need_head_weights=False, return_contacts=False):
		if return_contacts:
			need_head_weights = True
		
		if lengths:
			assert tokens.size(0) == len(lengths)
		...
		if need_head_weights:
			attn_weights = [[] for _ in range(len(lengths))]
		...
		for layer_idx, layer in enumerate(self.layers):
			...
			if need_head_weights:
				...
				for i, length in enumerate(lengths):
					attn_weights[i].append(attn[i, :, 1:length + 1, 1:length + 1])
		...
		if need_head_weights:
			# attention: L x H x T x T
			attention_list = []
			for attention in attn_weights:
				attention_list.append(torch.stack(attention, 0))
			...
			result["attentions"] = attention_list
			if return_contacts:
				contacts = self.contact_head(tokens, attention_list)
				result["contacts"] = contacts
		
		return result
	
	def predict_contacts(self, tokens, lengths):
		return self(tokens, return_contacts=True, lengths=lengths)["contacts"]
