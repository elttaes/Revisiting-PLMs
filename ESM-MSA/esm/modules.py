...
class ContactPredictionHead(nn.Module):
    ...
    def forward(self, tokens, attentions):
        ...
        for attn in attentions:
            layers, heads, seqlen, _ = attn.size()
            attn = attn.view(layers * heads, seqlen, seqlen).to(torch.float32)
            attn = apc(symmetrize(attn))
            attn = attn.permute(1, 2, 0)
            map = self.activation(self.regression(attn).squeeze(-1))
            maps.append(map)
