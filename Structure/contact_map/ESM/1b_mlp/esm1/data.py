...
esm1b_alphabet = Alphabet.from_architecture('ESM-1b')
...
class ESMStructuralSplitDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with open(pkl_fname, "rb") as f:
            obj = pickle.load(f)
        a3m_name = os.path.join('/root/ssp/msa_transormer_sujindata/structural-data/msas',name+'.a3m')
        msa=read_sequence(a3m_name)
        msa768=[]
        msa768.append(msa[0])
        msa768.append(msa[1][:512])
        contact_map = np.less(obj['dist'],8.0).astype(np.int64)
        return msa768,contact_map[:512,:512],len(msa768[1])
    
    def collate_fn(self, batch: List[Tuple[Any, ...]]) -> Dict[str, torch.Tensor]:
        input_ids, contact_labels, protein_length = tuple(zip(*batch))
        msa_batch_label, seq_str,msa_batch_token = batch_converter(input_ids)
        input_ids = msa_batch_token
        contact_labels = pad_sequences(contact_labels, -1)
        protein_length = torch.LongTensor(protein_length)
